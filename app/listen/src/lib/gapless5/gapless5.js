 ...
 * Original: https://github.com/regosen/Gapless-5
 * License: MIT (see LICENSE.txt in the upstream repo)
 *
 * --------------------------------------------------------------------
 * Forked into the Crate repo (src/lib/gapless5/) so local patches
 * aren't wiped by `npm install` and can be maintained alongside
 * project code. Local patches:
 *
 *   1. Added player.masterOut (GainNode) sitting between every source's
 *      gainNode and context.destination. All sources now route through
 *      this single point so we can insert post-processing (equalizer,
 *      compressor, etc.) without touching each Source.
 *
 *   2. Added player.setOutputChain(inputNode, outputNode) to splice a
...
function Gapless5Source(parentPlayer, parentLog, inAudioPath) {
  ...
  this.setCrossfade = (amountIn, amountOut, resetEndedCallback = true) => {
    ...
    if (endpos > 0) {
      ...
      if (totalFade > endpos) {
        log.warn(
          `Crossfade total exceeds duration (${totalFade} > ${endpos}), clamping for ${this.audioPath}`,
        );
  ...
  const onLoadedWebAudio = (inBuffer) => {
    ...
    if (!gainNode) {
      ...
      // [vendored patch] route through player.masterOut instead of
      // jumping straight to destination, so setOutputChain() can splice
      // post-processing (e.g. equalizer) between masterOut and output.
      gainNode.connect(player.masterOut || player.context.destination);
    }
    ...
    if (seekablePercent < 1) {
      ...
      if (player.useWebAudio && [Starting, Play, Stop].includes(state)) {
        ...
        if (!Number.isFinite(audioDuration) || audioDuration <= 0) {
          ...
          seekablePercent =
            audio.seekable.end(audio.seekable.length - 1) / audioDuration;
      ...
      if (audioPath.startsWith("blob:")) {
        ...
        const xhrFailSafe = (reason) => {
          ...
          if (audio !== null && state === Gapless5State.Play) {
            log.warn(
              `WebAudio fetch ${reason} for ${audioPath}; staying on HTML5`,
            );
            request = null;
            return;
          }
...
function Gapless5(options = {}, deprecated = {}) {
  ...
  // [vendored patch] Master output node — every source's gainNode
  // connects here instead of straight to destination. Lets a caller
  // insert an effect chain (EQ, compressor, etc.) via setOutputChain.
  if (this.context) {
    this.masterOut = this.context.createGain();
    this.masterOut.connect(this.context.destination);
    // Track whatever we're splicing between masterOut and destination so
    // setOutputChain(null) can clean it up without leaks.
    this._outputChainInput = null;
    this._outputChainOutput = null;
  }

  /**
   * [vendored patch] Splice a processing chain between the player's
   * masterOut and the AudioContext destination.
   *
   *   source(s) → gainNode → masterOut → inputNode → ... → outputNode → destination
   *
   * Pass (null, null) to remove the current chain and restore direct
   * output.
   *
   * @param {AudioNode | null} inputNode
   * @param {AudioNode | null} outputNode
   */
  this.setOutputChain = (inputNode, outputNode) => {
    ...
    if (this._outputChainOutput) {
      try {
        this._outputChainOutput.disconnect(this.context.destination);
      } catch {
        /* not connected */
      }
    }
    if (inputNode && outputNode) {
      this.masterOut.connect(inputNode);
      outputNode.connect(this.context.destination);
      this._outputChainInput = inputNode;
      this._outputChainOutput = outputNode;
    } else {
      this.masterOut.connect(this.context.destination);
      this._outputChainInput = null;
      this._outputChainOutput = null;
   ...
   */
  this.getTrack = () =>
    this.currentSource() ? this.currentSource().audioPath : "";
  ...
  this.isPlaying = () =>
    (this.currentSource() && this.currentSource().inPlayState()) || false;
  ...
  if (guiElement) {
    ...
    if (window.gapless5Players && this.id in window.gapless5Players) {
      guiElement.insertAdjacentHTML(
        "beforeend",
        createGUI(`window.gapless5Players['${this.id}']`),
      );
    }
...
// [vendored patch] Upstream used a UMD wrapper that relied on top-level
// `this` being the global object. In an ES module `this` is undefined,
