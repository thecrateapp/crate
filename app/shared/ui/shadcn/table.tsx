...
function Table({ className, ...props }: React.ComponentProps<"table">) {
  return (
    <div
      ...
      className="relative w-full overflow-x-auto rounded-md border border-text-primary/8 bg-surface-canvas/10 shadow-[0_16px_40px_rgba(0,0,0,0.18)] backdrop-blur-sm"
    >
      <table
        data-slot="table"
...
function TableRow({ className, ...props }: React.ComponentProps<"tr">) {
  return (
    <tr
      data-slot="table-row"
      className={cn(
        "border-b border-text-primary/6 transition-colors hover:bg-text-primary/[0.04] data-[state=selected]:bg-primary/8",
        className,
      )}
      {...props}
    />
